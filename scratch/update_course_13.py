...
# Get course 13
cursor.execute("SELECT content FROM courses WHERE id=13")
...
# Replace Section 4 onwards
index = content.find("## 4. Outils et Techniques de la GPI")
if index != -1:
    new_content = content[:index] + new_section_4
    cursor.execute("UPDATE courses SET content = ? WHERE id = 13", (new_content,))
    conn.commit()
    print("Updated Course 13 content!")
else:
    print("Could not find section 4 heading in content.")
